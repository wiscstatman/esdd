...
From Figure \ref{fig:F1} below, Adaptive Selection (AS8) is also the best method other than AC-BOISE and BOISE under F1 Score. After paired t-test previously mentioned, the mean difference between AS8 and AC-BOISE is $0.0433$ with p-value $=1.783\times 10^{-5}$; and  mean difference between AS8 and BOISE is $0.0480$ with p-value $=1.637\times 10^{-6}$:

\begin{figure}[!ht]
\caption{\label{fig:F1} 
{\bf Violin plot comparison of different methods under informer size $n_A =8$ with respect to compound ranking performance assessed by F1 Score.} Each method is evaluated through leave one target out cross validation of PKIS1 targets. F1 Score takes value in $[0,1]$. F1 Score of $0$ indicates a random guess, and F1 Score of 1.0 represents ideal ranking with all active compounds prioritized above the inactives. The median and interquartile ranges are displayed as a white circle and black bars, respectively.} 
\centering
\includegraphics[width=5.0in]{pkis1_8_F1.png}
\end{figure}
